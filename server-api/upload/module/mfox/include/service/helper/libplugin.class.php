...
define('PHPFOX_DIR_PLUGIN_MFOX', PHPFOX_DIR_CACHE . 'plugin_mfox' . PHPFOX_DS);
...
class Phpfox_Plugin
{
	...
	{	
		if (!is_dir(PHPFOX_DIR_PLUGIN_MFOX))
		{
			mkdir(PHPFOX_DIR_PLUGIN_MFOX);
			chmod(PHPFOX_DIR_PLUGIN_MFOX, 0777);
		}
	}
	...
	{		
		...
		$iCacheId = $oCache->set(array('plugin', 'plugin_mfox_plugins'));
		self::$_aPlugins = $oCache->get($iCacheId);
		...
		if(!(self::$_aPlugins))
		{
			...
			foreach ($aModules as $sModule => $iModuleId)
			{
				if (is_dir(PHPFOX_DIR_MODULE . $sModule . PHPFOX_DS . PHPFOX_DIR_MODULE_PLUGIN . PHPFOX_DS))
				{
			       	...
			       	while (($sHook = readdir($rHooks)) !== false)
					{
						...
						// check existing 
						// if (is_file(PHPFOX_DIR_PLUGIN_MFOX . 'plugin_data_' . $sHookVarName . '.php'))
						// {
						// 	continue;
						// }
			...
			{			
				...
				$iPluginCacheId = $oCache->set(array('plugin_mfox', 'plugin_data_' . $sKey));
	...
	public static function get($sCallName)
	{
		...
		{			
			$oCache = Phpfox::getLib('cache');
			$iCacheId = $oCache->set(array('plugin_mfox', 'plugin_data_' . self::$_aPlugins[$sCallName]));
