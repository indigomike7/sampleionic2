 ...
 */
class Mfox_Service_Userbrowse extends Phpfox_Service
{
	...
	public function get()
	{
		...
		if ($iCnt > 0)
		{
			...
			if($lat && $lon){
				$distance =  "( 3959 * acos( cos( radians($lat) ) * cos( radians( u.latitude ) ) 
	* cos( radians( u.longitude ) - radians($lon) ) + sin( radians($lat) ) * sin(radians(u.latitude)) ) ) AS distance";
				$aReturnUsers = $this->database()
				->select('u.status_id as unverified, '.$distance.', ' . ($this->_bExtend ? 'u.*, ufield.*' : Phpfox::getUserField()))
					->from($this->_sTable, 'u')
					->join(Phpfox::getT('user_field'), 'ufield', 'ufield.user_id = u.user_id')
					->where($this->_aConditions)
					->order('distance asc, u.last_activity desc')
					->limit($this->_iPage, $this->_iLimit, $iCnt)
					->group('u.user_id')
					->execute('getSlaveRows');
			}else{
				$aReturnUsers = $this->database()
					->select('u.status_id as unverified, ' . ($this->_bExtend ? 'u.*, ufield.*' : Phpfox::getUserField()))
					->from($this->_sTable, 'u')
					->join(Phpfox::getT('user_field'), 'ufield', 'ufield.user_id = u.user_id')
					->where($this->_aConditions)
