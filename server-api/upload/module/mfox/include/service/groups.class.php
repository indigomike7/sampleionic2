...
class Mfox_Service_Groups extends Phpfox_Service
{
	...
	function __construct(){
		$this->_sDefaultImagePath =  Phpfox::getParam('core.path_file') . 'module/mfox/static/image/pages_default.png';
		$this->_sDefaultCoverImagePath =  Phpfox::getParam('core.path_file') . 'module/mfox/static/image/pages_default.png';
	}
    ...
    public function doGroupsGetNotificationComment_Feed($aNotification)
    {
    ...
    public function setGroupsView($status = true){
        define('PHPFOX_IS_GROUP_VIEW', $status);
    }
    ...
    /**
     * @param $aData['iGroupId']
     * @return array
     */
    public function delete($aData){
        $iGroupId = isset($aData['iGroupId']) ? (int) $aData['iGroupId'] : 0;
        if ((int)$iGroupId < 1)
        {
            return array('result' => 0, 'error_code' => 1, 'error_message' => 'Group id is not valid!');
        }

        $aGroup = Phpfox::getService('groups')->getForView($iGroupId);
        if (!($aGroup) || !isset($aGroup['page_id']))
        {
            return array('result' => 0, 'error_code' => 1, 'error_message' =>  _p('The group you are looking for cannot be found.'));
        }

        if (Phpfox::getUserBy('profile_page_id') <= 0 && Phpfox::isModule('privacy'))
        {
            if(!Phpfox::getService('privacy')->check('pages', $aGroup['page_id'], $aGroup['user_id'], $aGroup['privacy'], (isset($aGroup['is_friend']) ? $aGroup['is_friend'] : 0), true)){
        ...
        if (Phpfox::getService('groups.process')->delete($iGroupId))
        {
            return array('result' => 1
                , 'error_code' => 0
                , 'message' =>  _p('Group successfully deleted')
            );
        }

        return array('result' => 0
            , 'error_code' => 1
            , 'error_message' => _p('Group has been deleted or you do not have permission to delete it!')
        );
    }

    public function fetch($aData){
        return $this->getGroups($aData);
    }
    ...
    public function getGroups($aData){
        ...
        {
            $aType = Phpfox::getService('groups.type')->getById($iCategoryId);
        ...
        switch ($sView) {
            ...
            //     break;
            case 'my':
                $aCond[] = ' pages.item_type = 1 AND pages.app_id = 0 AND pages.view_id IN(0,1) AND pages.user_id = ' . Phpfox::getUserId();
                break;
            case 'pending':
                ...
                $aCond[] = ' pages.item_type = 1 AND pages.app_id = 0 AND pages.view_id = 1';
                break;
            default:
                if (Phpfox::getUserParam('privacy.can_view_all_items'))
                {
                    $aCond[] = ' pages.item_type = 1 AND pages.app_id = 0';
                }
                else
                {
                    $aCond[] = ' pages.item_type = 1 AND pages.app_id = 0 AND pages.view_id = 0 AND pages.privacy IN(0)';
                }
                break;
        }
        ...
        foreach ($aRows as $aItem)
        {
			...
			if(Phpfox::getUserId() == $aItem['user_id']){
				...
				$bIsAdmin =  1;
				$bIsOwner = Phpfox::getService('groups')->isAdmin($aItem['page_id']);
			}
    ...
    public function canView($iItemId){
        if (!user('pf_group_browse'))
        {
            return array('result' => 0, 'error_code' => 1, 'error_message' => _p('You do not have permission to access group!'));
        }

        if (!($aItem = Phpfox::getService('groups')->getForView($iItemId)))
        {
            return array('result' => 0, 'error_code' => 1, 'error_message' => _p('The groups you are looking for cannot be found.'));
        }
        if (user('pf_group_moderate', '0') || $aItem['is_admin'])
        {
        ...
        if(Phpfox::isModule('privacy') && !Phpfox::getService('privacy')->check('groups', $aItem['page_id'], $aItem['user_id'], $aItem['privacy'], (isset($aItem['is_friend']) ? $aItem['is_friend'] : 0), true)){
            return array('result' => 0, 'error_code' => 1, 'error_message' =>  Phpfox::getPhrase('privacy.the_item_or_section_you_are_trying_to_view_has_specific_privacy_settings_enabled_and_cannot_be_viewed_at_this_time'));
        }
        $bHasPermToViewPageFeed = true;
        if ($aItem['reg_method'] == 2 && !Phpfox::getService('groups')->isMember($aItem['page_id'])){
            if (!Phpfox::getService('groups')->isInvited($aItem['page_id'])){
                $bHasPermToViewPageFeed = false;
            }
        }
        if($bHasPermToViewPageFeed == false){
            return array('result' => 0, 'error_code' => 1, 'error_message' => _p('You do not have permission to access groups!'));
        }
    ...
    public function canViewActivity($iItemId){
        if (!user('pf_group_browse'))
        {
            return array('result' => 0, 'error_code' => 1, 'error_message' => _p('You do not have permission to access group!'));
        }
        ...
        $bCanViewActivity = true;
        if (!Phpfox::isAdmin() && isset($aItem['page_id']) && isset($aItem['reg_method'])){
            if (!Phpfox::getService('groups')->isMember($aItem['page_id']) && in_array($aItem['reg_method'], [1, 2])){
                $bCanViewActivity = false;
            }
        }
        if($bCanViewActivity == false){
            return array('result' => 0, 'error_code' => 1, 'error_message' => _p('Unable to view this section due to privacy settings.'));
        }
        return null;
    }
    ...
    public function getMembers($aGroup = null, $iGroupId = null){
        if(null == $aGroup){
            $aGroup = Phpfox::getService('groups')->getForView((int)$iGroupId);
        }

        $aMembers = array();
        if ($aGroup['page_type'] == '1')
        {
            list($iTotalMembers, $aMembers) = Phpfox::getService('groups')->getMembers($aGroup['page_id']);
        ...
        return array($aGroup['total_like'], $aLikes);
    }
    ...
    public function info($aData){
        ...
        if ($iGroupId < 1)
        {
            return array('result' => 0, 'error_code' => 1, 'error_message' => _p('Group id is not valid!'));
        }
		...
		if($aGroup['image_path']){
				$sAvatarImage = Phpfox::getLib('image.helper')->display(array(
                'server_id' => $aGroup['profile_server_id'],
                'path' => 'pages.url_image',
        ...
        // get cover
        $aCoverPhoto = $this->getCoverPhoto($aGroup['cover_photo_id']);
		$bHasCover =  0;
        $sCoverImage = '';
        if(isset($aCoverPhoto['photo_id'])){
        	$bHasCover = 1;
        ...
        // get member
        list($iTotalMembers, $aMembers) = $this->getMembers($aGroup);         

        // get admins
        $aAdmins = $this->getAdminsByGroupId($aGroup['page_id'], $aGroup);
        $iTotalAdmins = count($aAdmins);

        // extra info
        $bCanView = $this->canView($aGroup['page_id']);
        if(isset($bCanView['error_code']) == false){
            $bCanView = true;
        } else {
            $bCanView = false;
        }
        $bCanComment = $this->canPostComment($aGroup['page_id']);
        ...
        $bCanViewActivity = $this->canViewActivity($aGroup['page_id']);
        if(isset($bCanComment['error_code']) == false){
            $bCanViewActivity = true;
        } else {
            $bCanViewActivity = false;
        ...
        $bIsInvited = $this->isInvited($aGroup['page_id'], Phpfox::getUserId());
        ...
        list($bCanShareBlogs, 
            ...
            $bCanInvite) = $this->getPermissions($aGroup);

        $aUserDislike = array();
        $aDislike = Phpfox::getService('mfox.like')->getListOfDislikeUser('pages', $aGroup['page_id'], $bGetCount = false);
        foreach($aDislike as $dislike){
            $aUserDislike[] = array('iUserId' => $dislike['user_id'], 'sDisplayName' => $dislike['full_name']);
        }   
        $iTotalDislike = count($aUserDislike);    
		...
		if(Phpfox::getUserId() == $aGroup['user_id']){
			...
			$bCanEdit =  1;
			$bCanDelete = 1;
		}else if(Phpfox::getService('groups')->isAdmin($iGroupId)){
			$bIsAdmin =  1;
			$bCanEdit =  1;
			$bCanDelete = 0 ;
		}
        ...
        $aReturn = array(
            ...
            'iUserId' => $aGroup['user_id'], 
            'sTitle' => $aGroup['title'], 
            'sText' => Phpfox::getLib('phpfox.parse.output')->parse($aGroup['text']),
            'sAvatarImage' => $sAvatarImage . '?t=' . time(), 
            'sCoverImage' => $sCoverImage, 
            'bHasCover'=>$bHasCover,
            'sCategoryName' => Phpfox::isPhrase($aGroup['category_name']) ? Phpfox::getService('mfox')->decodeUtf8Compat(Phpfox::getPhrase($aGroup['category_name'])) : Phpfox::getService('mfox')->decodeUtf8Compat(Phpfox::getLib('locale')->convert($aGroup['category_name'])), 
            'iTotalMembers' => $iTotalMembers, 
            'aMembers' => $aMembers, 
            'iTotalAdmins' => $iTotalAdmins, 
            'aAdmins' => $aAdmins, 
            'bIsOwner' => ($aGroup['user_id'] == Phpfox::getUserId() ? true : false), 
            'bCanView' => $bCanView, 
            'bCanComment' => $bCanComment, 
            'bCanViewActivity' => $bCanViewActivity,
            'bIsLiked' => $bIsLiked,
            ...
            'iTypeId' => $aGroup['type_id'], 
            'bHasImage'=>$aGroup['image_path']?1:0,
            'iCategoryId' => $aGroup['category_id'], 
            'bCanDislike' => Phpfox::getService('mfox.like')->isTypeSupportDislike('pages'),
            'bIsDisliked' => Phpfox::getService('mfox.like')->checkIsDisliked('pages', $aGroup['page_id'], Phpfox::getUserId()),
            'aUserDislike' => $aUserDislike,
            'iTotalDislike' => $iTotalDislike,
            'bCanEdit'=>$bCanEdit,
            'bCanDelete'=>$bCanDelete,
            'bIsOwner'=>$bIsOwner,
            'bCanLike' => $this->_canLike($aGroup),
            'bCanUnlike' => $this->_canUnlike($aGroup),
            'iPageType' => (int) $aGroup['page_type'],
            'iRegMethod' => $aGroup['reg_method'],
            'bCanShowAdmin' => setting('pf_group_show_admins'),
        );
    ...
    private function _canUnlike($aGroup)
    {
        $result = false;
    ...
    private function _canLike($aGroup)
    {
        $result = false;
        
        if (!Phpfox::getUserBy('profile_page_id') && Phpfox::isUser()) {
            if ($aGroup['reg_method'] == '2' && !isset($aGroup['is_invited']) && $aGroup['page_type'] == '1') {
            } else {
                if (isset($aGroup['is_reg']) && $aGroup['is_reg']) {
                } else {
                    if (isset($aGroup['is_liked']) && $aGroup['is_liked'] != true) {
                        $result = true;
                    }
                }
            }
    ...
    public function getinvitepeople($aData){
        ...
        $aGroup = Phpfox::getService('groups')->getPage($iGroupId);
        if (!($aGroup) || !isset($aGroup['page_id']))
        {
        ...
        $aConds =  array();
        $aInvitedIds = $this->getInvitedUserIds($iGroupId);
    ...
    public function invite($aData){
        ...
        {            
            $sMessage =  _p('{{ full_name }} invited you to the group "{{ title }}".', array('full_name' => Phpfox::getUserBy('full_name'), 'title' => $aGroup['title']));
            $sMessage .= "\n" .  _p('To view this group click the link below: <a href="{{ link }}">{{ link }}</a>', array('link' => $sLink)) . "\n";
        
            $bSent = Phpfox::getLib('mail')->to($aUser['user_id'])                      
                ->subject(_p('{{ full_name }} sent you a page invitation.', array('full_name' => Phpfox::getUserBy('full_name'))))
                ->message($sMessage)                    
                ->send();
            ...
            {                   
                ...
                (Phpfox::isModule('request') ? Phpfox::getService('request.process')->add('groups_invite', $iGroupId, $aUser['user_id']) : null);
            }
        }
    ...
    public function getPermissions($aGroup){
        $bCanShareBlogs = Phpfox::getService('groups')->hasPerm($aGroup['page_id'], 'blog.share_blogs');
        $eventModule = Phpfox::getService('mfox.event')->isAdvancedModule() ? 'fevent' : 'event';
        $bCanShareEvents = Phpfox::getService('groups')->hasPerm($aGroup['page_id'], $eventModule . '.share_events');
        $bCanShareMusic = Phpfox::getService('groups')->hasPerm($aGroup['page_id'], 'music.share_music');
        $photoModule = Phpfox::getService('mfox.photo')->isAdvancedModule() ? 'advancedphoto' : 'photo';
        $bCanSharePhotos = Phpfox::getService('groups')->hasPerm($aGroup['page_id'], $photoModule . '.share_photos');
        $bCanShareVideos = Phpfox::getService('groups')->hasPerm($aGroup['page_id'], 'video.share_videos');
        $bCanShareVideochannel = Phpfox::getService('groups')->hasPerm($aGroup['page_id'], 'videochannel.share_videos');

        $bCanInvite = false;
        if(user('pf_group_moderate', '0') || $aGroup['user_id'] == Phpfox::getUserId()){
            $bCanInvite = true;
        }
    ...
    public function isInvited($iGroupId, $iUserId){
        $count = (int) $this->database()->select('COUNT(*)')
                ->from(Phpfox::getT('pages_invite'))
                ->where('page_id = ' . (int) $iGroupId . ' AND invited_user_id = ' . $iUserId)
    ...
    public function getAdminsByGroupId($iGroupId, $aGroup = array()){
        $aRow = $this->database()->select('u.user_image, ' . Phpfox::getUserField())
            ...
            ->where('pa.page_id = ' . (int) $iGroupId)
            ->execute('getSlaveRows');

        if(isset($aGroup['page_id']) == false){
            $aGroup = Phpfox::getService('groups')->getForView($iGroupId);
        }
        $aOwnerAdmin = array();
        foreach ($aGroup as $sKey => $mValue)
        {
    ...
    public function edit_cover($aData, $aGroup = null){
        ...
        {           
            return array('result' => 0, 'error_code' => 1, 'error_message' => _p('No image to upload.'));
        }
        // Make sure the user group is actually allowed to upload an image
        if (!Phpfox::getUserParam('photo.can_upload_photos'))
        {           
            return array('result' => 0, 'error_code' => 1, 'error_message' => _p('You cannot upload photo.'));
        }
        ...
        $aVals = array(
            ...
            'is_cover_photo' => '1', 
            'page_id' => $isEdit ? $aGroup['page_id'] : 0, 
        );
        ...
        if (count($aImages)){
            ...
            if (isset($aVals['page_id']) && $aVals['page_id'] > 0){
                if (Phpfox::getService('groups.process')->setCoverPhoto($aVals['page_id'], $iId, true))
                 {
                    // upload successfully
    ...
    public function update_avatar($aData, $aGroup = null){
        $sAction = isset($aData['sAction']) ? $aData['sAction'] : 'add';
        $isEdit = true;
        if('add' == $sAction){
            $isEdit = false;
        ...
        // process image 
        if($bHasImage){
            $iId = $isEdit ? $aGroup['page_id'] : time();
            if($isEdit){
                ...
                {
                    Phpfox::getService('pages.process')->deleteImage($aGroup);
                }
            }
    ...
    public function create($aData){
        if (!user('pf_group_add'))
        {
            return array('result' => 0, 'error_code' => 1, 'error_message' => _p('You do not have permission to create new group!'));
        }
        ...
        if(strlen(trim($sTitle)) == 0){
            return array('result' => 0, 'error_code' => 1, 'error_message' => _p('Fill in a title for your group'));
        }
        if($iTypeId == 0){
            return array('result' => 0, 'error_code' => 1, 'error_message' => _p('Please chose category'));
        }
        ...
        $iId = Phpfox::getService('groups.process')->add($aInsert);
        if (!$iId)
        {
        ...
        // process cover image 
        if($iCovePhotoId > 0){
            Phpfox::getService('groups.process')->setCoverPhoto((int) $iId, $iCovePhotoId, true);
        }
    ...
    public function edit($aData){
        ...
        // get data
        Phpfox::getService('groups')->setIsInPage();
        $iGroupId = isset($aData['iGroupId']) ? $aData['iGroupId'] : 0;
        $sTitle = isset($aData['sTitle']) ? $aData['sTitle'] : '';
        $sInfo = isset($aData['sInfo']) ? $aData['sInfo'] : '';
        $iTypeId = isset($aData['iTypeId']) ? (int)$aData['iTypeId'] : 0;
        $iCategoryId = isset($aData['iCategoryId']) ? (int)$aData['iCategoryId'] : 0;
        ...
        $this->database()->update(Phpfox::getT('pages'), $aUpdate, 'page_id = ' . (int) $iGroupId);
        $this->database()->update(Phpfox::getT('pages_text'), array(
            'text' => $this->preParse()->clean($sInfo), 
            'text_parsed' => $this->preParse()->prepare($sInfo)
        ), 'page_id = ' . (int) $iGroupId);      

        // process avatar image 
        if(strlen(trim($sAvatarFileName)) > 0){
            $aUser = $this->database()->select('user_id')
                ->from(Phpfox::getT('user'))
                ->where('profile_page_id = ' . (int) $iGroupId)
                ->execute('getSlaveRow');                
        ...
        // process cover image 
        if($iCovePhotoId > 0){
            Phpfox::getService('groups.process')->setCoverPhoto((int) $iGroupId, $iCovePhotoId, true);
        }

        // end 
        $sMessage = _p('Your group has been updated');
        return array(
            'result' => 1,
            'error_code' => 0,
            'message' => $sMessage,
    ...
    public function categories($aData){
        $aCategories = Phpfox::getService('groups.category')->getCategories();
    ...
    public function addFeedComment($aData){
        ...
        $aError = $this->canPostComment($aGroup['page_id']);
        if ($aError)
        {
            return array(
                'result' => 0,
        ...
        if (isset($aVals['user_status']) 
            ...
            Phpfox::getLib('database')->updateCounter('pages', 'total_comment', 'page_id', $aGroup['page_id']);      
