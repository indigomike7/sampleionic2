...
class Mfox_Service_Musicsharing_Album extends Phpfox_Service {
    ...
    private $_oUserSettings;
	

    public function __construct(){
        $this->_sDefaultImageAlbumPath = Phpfox::getParam('core.url_module') . 'mfox/static/image/music_cover_default.png';
        $this->_oUserSettings = phpFox::getService('musicsharing.music')->getUserSettings(phpFox::getUserId(), true);
    }
    ...
    {
        if (!$this->_oUserSettings['can_view_album'])
        {
	...
	public function _parseAlbumItem($aRow){
		...
		return array(
		    ...
		    'bShowRate'=>false,
		    "iLanguageId"=> "en",
		    "sLink_Url" => Phpfox::getLib('url')->makeUrl('musicsharing.listen', array('album' => $aRow['album_id'])),
		);
	}
