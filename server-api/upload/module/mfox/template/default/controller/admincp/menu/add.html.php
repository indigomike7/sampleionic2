...
?>
<form class="mfox-menu-add" method="post" action="{url link='admincp.mfox.menu.add'}">
{if $bIsEdit}
    <div><input type="hidden" name="id" value="{$aForms.id}" /></div>
{/if}
    <div class="table_header">
        {phrase var='mfox.menu_details'}
    </div>
    ...
    <div class="table form-group">
        <div class="table_left">
            {phrase var='mfox.group'}:
        </div>
        <div class="table_right">
            <select name="val[parent_id]" style="width:300px;">
                <option value="0">{phrase var='mfox.select'}:</option>
                {$sOptions}
            </select>
        </div>
        <div class="clear"></div>
